...
# ── Thread 1: Camera — drains buffer aggressively ──
def camera_thread():
    global latest_frame
    ...
    while True:
        # Read 8 frames, keep only latest — kills lag
        frame = None
        for _ in range(8):
            ret, f = cap.read()
            if ret:
...
def recognition_thread():
    ...
    while True:
        ...
        print(f"  YOLO: {len(detections)} faces")
        ...
        for det in detections:
            ...
            if xyxy_np.shape != (4,) or conf < 0.30:
                continue
